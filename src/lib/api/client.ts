...
export async function apiFetch<T>(
  ...
  const { token: explicitToken, revalidate, ...rest } = init ?? {}

  // Pass a token explicitly (e.g. from a server action that just minted one)
  // or let it read the session cookie.
  const token =
    explicitToken !== undefined ? explicitToken : await getSessionToken()
